...
const saveOutcome = persistAnalysisToHistory(sample, report);
...
let loaded = loadPersistedHistory();
...
const duplicateOutcome = persistAnalysisToHistory(sample, report);
assert(duplicateOutcome.status === "duplicate", "duplicate save should be rejected");
loaded = loadPersistedHistory();
...
resetBrowserHistoryRepository();
const reloaded = loadPersistedHistory();
...
const verified = verifyPersistedMatch(saveOutcome.record.id, {
  fullTimeHomeGoals: 2,
  fullTimeAwayGoals: 1,
  halfTimeHomeGoals: 1,
  halfTimeAwayGoals: 0,
...
assert(verified?.status === "VERIFIED", "status should become VERIFIED");
assert(verified?.verificationResult !== null, "verification result should be saved");
assert(verified?.result?.winner === "home", "result should be saved");

const afterVerify = loadPersistedHistory();
